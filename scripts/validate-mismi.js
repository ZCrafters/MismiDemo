...
const p = JSON.parse(fs.readFileSync(new URL("../data/products.mismi.json", import.meta.url), "utf-8"));
const slugs = new Set();
const ids = new Set();
for (const it of p) {
  ...
  if (!img || !(/^https:\/\//.test(img) || img.startsWith("/"))) throw new Error("image invalid: " + it.slug);
