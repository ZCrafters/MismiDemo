...
### langkah 1a — Kumpulkan data real
- Buka halaman produk di marketplace (utamakan yang bisa diakses: biasanya Tokopedia).
- Untuk tiap produk catat: `name, category, pack, price, originalPrice, discountPercent, rating,
  soldCount/soldLabel, bpom, claim, image[url], sourceUrl`.
- **PENTING:** jangan mengarang harga/klaim/stok. Semua dari listing resmi.
- Marketplace lain (Shopee/Lazada/IG/TikTok) sering anti-bot → pakai data yang bisa diakses saja,
  atau minta user kirim manual. Dokumentasikan keterbatasan ini di CONTEXT.md.

### langkah 1b — Bangun file data produk
- Karena data sering dikurasi manual, tulis langsung ke `data/products.{{slug}}.json`
...
- Setiap item wajib berisi field seperti di CONTEXT.md "Skema SKU".

### langkah 1c — Script validasi & audit
...
- Buat `scripts/make-{{slug}}-audit.js` (exceljs): sheet Katalog (termasuk kolom **URL gambar**)
  + sheet Ringkasan. Output ke `{{FOLDER}}/{{slug}}-katalog-audit.xlsx`.
- Update `package.json` scripts: `validate` → `node scripts/validate-{{slug}}.js`,
  `audit` → `node scripts/make-{{slug}}-audit.js`.

```bash
node scripts/validate-{{slug}}.js   # → OK <N> produk (<8> hero), <M> kategori.
node scripts/make-{{slug}}-audit.js # → tulis xlsx (di-ignore git via *.xlsx)
```

### langkah 1d — Update helper `lib/products.js`
